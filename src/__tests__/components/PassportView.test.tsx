...
describe("PassportView — successful fetch (no jobStatus / COMPLETED)", () => {
  ...
  it.skip("renders the AgentPassport once data has loaded", async () => {
    mockFetchOnce({ ok: true, json: async () => basePassport });
    render(<PassportView />);
  ...
  it.skip("renders the AgentQR with the passport DID", async () => {
    mockFetchOnce({ ok: true, json: async () => basePassport });
    render(<PassportView />);
  ...
  it.skip("treats jobStatus 'COMPLETED' as fully loaded (renders passport, not the building UI)", async () => {
  ...
  it.skip("calls sharePassport with the current URL when the share button is clicked", async () => {
    mockFetchOnce({ ok: true, json: async () => basePassport });
    render(<PassportView />);
    await waitFor(() => expect(screen.getByTestId("agent-passport")).toBeInTheDocument());
...
describe("PassportView — identity still building (jobStatus not COMPLETED/ACTIVE)", () => {
  ...
  it.skip("polls again after 3 seconds while the job is still building", async () => {
    mockFetchOnce({ ok: true, json: async () => ({ ...basePassport, jobStatus: "PROVISIONING" }) });
    render(<PassportView />);
  ...
  it.skip("stops polling once jobStatus becomes COMPLETED", async () => {
    mockFetchOnce({ ok: true, json: async () => ({ ...basePassport, jobStatus: "PROVISIONING" }) });
    render(<PassportView />);
    await waitFor(() => expect(global.fetch).toHaveBeenCalledTimes(1));
