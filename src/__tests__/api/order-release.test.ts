...
jest.mock('@/lib/ip', () => ({
  getClientIp: jest.fn(() => '10.10.10.10'),
}));
...
describe('POST /api/marketplace/order/release', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  describe('rate limiting', () => {
    ...
    it('uses order-release key prefix with IP', async () => {
  ...
  describe('request validation', () => {
    it('returns 400 when paymentId is not a valid UUID', async () => {
      const req = makeRequest({ paymentId: 'invalid-id' });
      const res = await POST(req);
      const data = await res.json();
  ...
  describe('business logic', () => {
    ...
    it('updates payment to RELEASED status and returns status on success', async () => {
      mockPrisma.piPayment.findUnique.mockResolvedValue({
        id: PAYMENT_UUID, userId: AUTH_USER_ID, status: 'ESCROWED',
      } as any);
      mockPrisma.piPayment.update.mockResolvedValue({ id: PAYMENT_UUID, status: 'RELEASED' } as any);
    ...
    it('calls piPayment.update with RELEASED status', async () => {
      mockPrisma.piPayment.findUnique.mockResolvedValue({
        id: PAYMENT_UUID, userId: AUTH_USER_ID, status: 'ESCROWED',
      } as any);
      mockPrisma.piPayment.update.mockResolvedValue({} as any);
    ...
    it('release and refund use distinct status values (regression)', async () => {
      // Verify release sets RELEASED not REFUNDED
      mockPrisma.piPayment.findUnique.mockResolvedValue({
        id: PAYMENT_UUID, userId: AUTH_USER_ID, status: 'ESCROWED',
      } as any);
