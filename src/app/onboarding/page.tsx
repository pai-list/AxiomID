...
export default function OnboardingPage() {
  ...
  const stateConfigs: Record<OnboardingState, { label: string; colorClass: string }> = {
    VISITOR: { label: t("onboarding_state_visitor"), colorClass: "text-subtle border-border bg-surface-muted/30" },
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
          ...
          <div className="md:col-span-6 space-y-6">
            <div>
              ...
              <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-surface transition-all duration-300">
                {currentStepInfo().title}
              </h2>
              <p className="text-xs sm:text-sm text-faint mt-2 leading-relaxed">
              ...
              {[1, 2, 3, 4].map((i) => (
                <div 
                  ...
                  className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                    i <= step ? "bg-electric-blue" : "bg-surface-muted/30"
                  }`} 
                />
              ))}
            </div>
            ...
            <div className="bento-card p-6 bg-surface-deep/65 min-h-[160px] flex flex-col justify-between">
                ...
                {step === 2 && (
                  <div key="step-2-details" className="space-y-4 animate-fadeInUp">
                    <div className="space-y-2">
                      <label className="text-[10px] font-mono text-subtle uppercase block">{t("onboarding_agent_name_label")}</label>
                      <input
                        type="text"
                        placeholder={t("onboarding_agent_name_placeholder")}
                        value={agentName}
                        onChange={(e) => setAgentName(e.target.value)}
                        className="w-full bg-surface-muted/30 border border-border rounded-xl px-4 py-3 text-xs font-mono text-surface focus:outline-none focus:border-electric-blue/30 placeholder:text-subtle"
                      />
                    </div>
                    <button 
