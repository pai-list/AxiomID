    ...
    <main className="min-h-screen bg-grid relative pb-20">
      ...
      <div className="max-w-6xl mx-auto px-4 mt-8 relative z-10 grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <div className="md:col-span-3 space-y-2">
          <div className="p-3 text-[10px] font-mono text-faint uppercase tracking-widest">
            {t("docs_sidebar_label")}
          </div>

          {/* Search Bar */}
          <div className="relative">
            <Search className="w-3.5 h-3.5 text-faint absolute start-3 top-1/2 -translate-y-1/2" />
            <input
              type="text"
              placeholder={t("docs_search_placeholder")}
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-glass border border-glass-hover rounded-xl py-2.5 ps-9 pe-3 text-[11px] text-white font-mono placeholder-zinc-500 focus:outline-none focus:border-electric-blue/30 transition-colors"
            />
          </div>
            ...
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-4 py-3 rounded-xl text-left text-xs font-mono transition-all ${
                activeSection === sec.id
                  ...
                  : "text-faint hover:text-white hover:bg-glass border border-transparent"
              }`}
            >
              {sec.icon}
        ...
        <div className="md:col-span-9 bento-card p-6 md:p-8 min-h-[500px]">
          {activeSection === "intro" && (
            <div className="space-y-6">
              ...
              <h3 className="text-sm font-bold font-mono text-white pt-2 border-b border-glass pb-2">
                {t("docs_did_title")}
              </h3>
              <p className="text-faint text-sm leading-relaxed">
                {t("docs_did_desc")}
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3 pt-2">
                {[
                  ...
                  { name: t("docs_stamp_pow_name"), xp: t("docs_stamp_pow_xp"), desc: t("docs_stamp_pow_desc"), color: "text-faint" },
                ].map((stamp, i) => (
                  <div key={i} className="flex justify-between items-start p-3 rounded-xl border border-glass bg-white/[0.01] hover:bg-white/[0.02] transition-colors">
                    <div className="flex-1 min-w-0">
                      <h4 className={`text-xs font-bold font-mono ${stamp.color}`}>{stamp.name}</h4>
                      <p className="text-[11px] text-faint mt-1 leading-relaxed">{stamp.desc}</p>
                    </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  ...
                  <div key={i} className="p-3 rounded-xl border border-glass bg-white/[0.01]">
                    <h4 className="text-xs font-bold font-mono text-electric-blue">{pkg.name}</h4>
                    <p className="text-[11px] text-faint mt-1">{pkg.desc}</p>
              ...
              <h3 className="text-sm font-bold font-mono text-white pt-4 border-b border-glass pb-2">
                ...
                {[
                  ...
                  <div key={i} className="flex items-start gap-3 p-2 rounded-lg hover:bg-white/[0.01] transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-faint mt-0.5">{fn.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left font-mono text-xs">
                  <thead>
                    <tr className="border-b border-glass-hover text-[10px] text-faint">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {API_ROUTES.map((route, i) => (
                      <tr key={i} className="hover:bg-white/[0.01] transition-colors">
                        ...
                        <td className="py-2.5 pe-3 text-faint">{route.desc}</td>
                        <td className="py-2.5">
              ...
              <div className="bg-black/80 border border-glass rounded-xl p-3 font-mono text-[11px]">
