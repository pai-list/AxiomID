      ...
      >
        ...
        <div className="flex items-center justify-between z-10" style={{ transform: "translateZ(30px)" }}>
          <div className="flex items-center gap-2">
            ...
            <span className="font-mono text-xs font-semibold tracking-wider text-surface">AXIOM<span style={{ color: tierColor }}>ID</span></span>
          </div>
          <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-surface-muted/30 border border-border">
            <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: tierColor }} />
            <span className="text-[9px] font-mono tracking-widest text-subtle uppercase">{t("agent_passport")}</span>
          </div>
        </div>
        ...
        <div className={`flex items-center gap-4 my-6 z-10 transition-all ${locked ? "opacity-30 blur-[1px]" : ""}`} style={{ transform: "translateZ(40px)" }}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              <h3 className="text-sm sm:text-base font-bold font-mono text-surface truncate">{displayUsername}</h3>
              {hasUser && (
            ...
            <div className="mt-2.5 flex flex-wrap gap-1.5">
              <span className={`px-2 py-0.5 rounded text-[8px] font-mono uppercase tracking-wider font-semibold border ${
                isKya 
                  ...
                  : "bg-surface-muted/30 text-faint border-border"
              }`}>
        ...
        {locked && (
          <div className="absolute inset-0 flex flex-col items-center justify-center z-30 pointer-events-none" style={{ transform: "translateZ(50px)" }}>
            <div className="w-10 h-10 rounded-full bg-surface-muted/30 border border-border flex items-center justify-center shadow-lg backdrop-blur-md">
              <Lock className="w-4 h-4 text-faint" />
            </div>
            <span className="text-[9px] font-mono text-faint uppercase tracking-widest mt-2 bg-surface-deep/60 px-2 py-1 rounded border border-border">
        ...
        <div className={`flex items-center justify-between border-t pt-4 z-10 transition-all ${locked ? "opacity-30" : ""}`} style={{ borderColor: "rgba(255, 255, 255, 0.08)", transform: "translateZ(30px)" }}>
          <div className="flex gap-4">
            <div>
              <span className="text-[8px] font-mono text-subtle block uppercase tracking-wider">{t("trust_score")}</span>
              <span className="text-xs sm:text-sm font-bold font-mono text-surface">{trustScore}%</span>
            </div>
            <div className="border-l border-border pl-4">
              <span className="text-[8px] font-mono text-subtle block uppercase tracking-wider">{t("xp_balance")}</span>
